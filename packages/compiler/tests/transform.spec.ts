...
async function getFormatedCodegenResult(
  ...
  compileVineTypeScriptFile(specContent, 'testTransformSeparatedResult', {
...
`
    ...
    compileVineTypeScriptFile(specContent, 'testUnusedImports', {
      compilerHooks: mockCompilerHooks,
    })
    expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)
...
export async function MyComp() {
  ...
  \`
    ...
    `
    compileVineTypeScriptFile(specContent, 'testTransformTopLevelAwait', {
      compilerHooks: mockCompilerHooks,
    })
    expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)
...
export default function MyComp() {
  ...
  \`
    ...
    compileVineTypeScriptFile(
      specContent,
      'testExportDefaultInCorrectPosition',
      { compilerHooks: mockCompilerHooks },
    )
    expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)
    const fileCtx = mockCompilerCtx.fileCtxMap.get(
...
export function MyComp({
  ...
  \`
    ...
    `
    compileVineTypeScriptFile(specContent, 'testTransformDestructuredProps', {
      compilerHooks: mockCompilerHooks,
    })
    expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)

    const fileCtx = mockCompilerCtx.fileCtxMap.get(
      'testTransformDestructuredProps',
    )
    const transformed = fileCtx?.fileMagicCode.toString() ?? ''
    const formated = await format(transformed, { parser: 'babel-ts' })
...
export {
  MyComp,
    ...
    compileVineTypeScriptFile(specContent, 'testExportOnFunctionDeclaration', {
      compilerHooks: mockCompilerHooks,
    })
    expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)
    const fileCtx = mockCompilerCtx.fileCtxMap.get(
      'testExportOnFunctionDeclaration',
    )
    const transformed = fileCtx?.fileMagicCode.toString() ?? ''
    const formated = await format(transformed, { parser: 'babel-ts' })
...
export function App() {
  ...
  \`
    ...
    compileVineTypeScriptFile(specContent, 'testTransformNegativeBoolProps', {
      compilerHooks: mockCompilerHooks,
    })
    expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)
    const fileCtx = mockCompilerCtx.fileCtxMap.get(
      'testTransformNegativeBoolProps',
    )
    const transformed = fileCtx?.fileMagicCode.toString() ?? ''
    const formated = await format(transformed, { parser: 'babel-ts' })
    expect(formated).toMatchSnapshot()
...
export function TestComponent(props: {
  ...
  \`
    ...
    compileVineTypeScriptFile(specContent, 'testKebabCaseProps', {
      compilerHooks: mockCompilerHooks,
    })
    expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)
...
export function TestComponent({
  ...
  \`
    ...
    compileVineTypeScriptFile(specContent, 'testKebabCasePropsWithDefaults', {
      compilerHooks: mockCompilerHooks,
    })
    expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)
    const fileCtx = mockCompilerCtx.fileCtxMap.get(
      'testKebabCasePropsWithDefaults',
    )
    const transformed = fileCtx?.fileMagicCode.toString() ?? ''
    const formated = await format(transformed, { parser: 'babel-ts' })
  ...
  it.each([
    ...
    'should generate correct code for export default anonymous function: $name',
    async ({ code }) => {
      const { mockCompilerCtx, mockCompilerHooks } = createMockTransformCtx()
      compileVineTypeScriptFile(code, 'testExportDefaultAnonymous', {
        compilerHooks: mockCompilerHooks,
      })
      expect(mockCompilerCtx.vineCompileErrors.length).toBe(0)
      const fileCtx = mockCompilerCtx.fileCtxMap.get(
