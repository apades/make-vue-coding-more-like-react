...
import { createCompilerCtx } from '../src'
...
export function createMockTransformCtx(
  ...
  const mockCompilerHooks = {
    ...
    onError: (err) => {
      mockCompilerCtx.vineCompileErrors.push(err)
    },
    onWarn: (warn) => {
      mockCompilerCtx.vineCompileWarnings.push(warn)
    },
    onBindFileCtx: (fileId, fileCtx) =>
      mockCompilerCtx.fileCtxMap.set(fileId, fileCtx),
