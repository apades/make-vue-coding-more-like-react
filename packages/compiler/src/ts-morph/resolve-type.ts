...
function isBooleanType(typeChecker: TypeChecker, type: Type): boolean {
  if (
    type.isBoolean() ||
    type.isBooleanLiteral() ||
    (type.isLiteral() &&
      (type.getLiteralValue() === 'true' || type.getLiteralValue() === 'false'))
  ) {
    return true
  }
  ...
  if (type.isUnion()) {
    return type.getUnionTypes().some((t) => isBooleanType(typeChecker, t))
  }
  ...
  if (type.isIntersection()) {
    return type
      ...
      .some((t) => isBooleanType(typeChecker, t))
  }
...
export function tsAstFindVineCompFn(
  vineCompFnName: string,
): Parameters<TsMorphNode['getFirstDescendant']>[0] {
  return (node) => {
    ...
    const body = Node.isFunctionDeclaration(node)
      ? node.getBody()
    ...
    const returnStatement = body.getFirstDescendant(
      (nodeInReturnStmt) =>
        ...
        (
          nodeInReturnStmt.getExpression() as
            | TaggedTemplateExpression
            | undefined
        )
          ?.getTag()
    ...
    const fnName = Node.isFunctionDeclaration(node)
      ? node.getName()
