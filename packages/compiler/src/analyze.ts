...
const analyzeVineProps: VineAnalyzeRunner = (
  ...
  if (formalParams.length === 1) {
    ...
    allVinePropMacroCalls.forEach(
      ({ macroCall, identifier: propVarIdentifier, jsDocComments }) => {
        // Collect prop's information
        const propName = propVarIdentifier.name
        const macroCalleeName = getVineMacroCalleeName(
          macroCall,
        ) as VINE_MACRO_NAMES
        const propMeta: VinePropMeta = {
        ...
        if (macroCalleeName === 'vineProp.withDefault') {
          ...
          propMeta.isMaybeBool = tsMorphCache
            ? checkForBoolean({
                propName,
                tsMorphCache,
                vineCompilerHooks,
        ...
        if (macroCallTypeParamNode) {
          const macroCallTypeParam = vineFileCtx.getAstNodeContent(
            macroCallTypeParamNode,
          )
...
const analyzeRunners: VineAnalyzeRunner[] = [
  analyzeVineProps,
...
function analyzeDifferentKindVineFunctionDecls(analyzeCtx: VineAnalyzeCtx) {
