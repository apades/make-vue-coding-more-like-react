...
import type { VineFileCtx, VineCompilerHooks, VineCompFnCtx } from '../types'
import {
...
import { removeRange, replaceRange } from '../utils/magic-string-helpers'
import {
  fineAllExplicitExports,
  isCallOf,
  isStatementContainsVineMacroCall,
...
export function createVueImportsSpecs(
  ...
  if (
    ...
    !vueImportsSpecs.has(DEFINE_CUSTOM_ELEMENT_HELPER)
  ) {
    vueImportsSpecs.set(
      DEFINE_CUSTOM_ELEMENT_HELPER,
      `_${DEFINE_CUSTOM_ELEMENT_HELPER}`,
    )
  }
  // add useCssVars
...
export function onlyRemainFunctionBody(
  ...
  const { vineCompFnStart, vineCompFnEnd, firstStmt, lastStmt } = fnTransformCtx
...
export function removeStatementsContainsMacro(
  ...
  vineCompFnBody.body.forEach((stmt) => {
    if (isStatementContainsVineMacroCall(stmt) || isReturnStatement(stmt)) {
...
export function generateVinePropToRefs(
  ...
  if (
    Object.values(vineCompFnCtx.props).some((meta) =>
      Boolean(meta.isFromMacroDefine),
    )
  ) {
    registerImport(
      mergedImportsMap,
...
export function generatePropsRestProxy(
  ...
  const propsRestEntry = Object.entries(
    vineCompFnCtx.propsDestructuredNames,
  ).find(([_, propMeta]) => propMeta.isRest)
  if (propsRestEntry) {
    vueImportsSpecs.set(
      CREATE_PROPS_REST_PROXY_HELPER,
      `_${CREATE_PROPS_REST_PROXY_HELPER}`,
    )
  }
  ...
  if (propsRestEntry) {
    ...
    ms.prependLeft(
      firstStmt.start!,
      `const ${propName} = _${CREATE_PROPS_REST_PROXY_HELPER}(__props, [${Object.entries(
        vineCompFnCtx.propsDestructuredNames,
      )
...
export function generatePropsDestructure(
  ...
  if (Object.keys(vineCompFnCtx.propsDestructuredNames).length > 0) {
    ...
    const destructuredNames = Object.entries(
      vineCompFnCtx.propsDestructuredNames,
  ...
  for (const [destructName, destructMeta] of Object.entries(
    vineCompFnCtx.propsDestructuredNames,
  )) {
...
export function generatePropsDeclaration(
  ...
  ms.prependLeft(firstStmt.start!, propsDeclarationStmt)
}
...
export function generateAsyncContext(
  ...
  for (const vineFnBodyStmt of vineCompFnBody.body) {
    ...
    replaceRange(
      ...
      wrapWithAsyncContext(
        isNeedResult,
        ms.original.slice(awaitExpr.argument.start!, awaitExpr.argument.end!),
      ),
    )
  }
  ...
  if (fnTransformCtx.hasAwait) {
    ms.prependLeft(firstStmt.start!, 'let __temp, __restore;\n')
  }
}
...
export function buildSetupFormalParams(
  ...
  const setupFormalParams = `__props${
    setupCtxDestructFormalParams.length > 0
      ? `, { ${setupCtxDestructFormalParams
          .map(({ field, alias }) => `${field}${showIf(!!alias, `: ${alias}`)}`)
          .join(', ')} }`
  ...
  ms.prependLeft(
    firstStmt.start!,
    `${vineCompFnCtx.isAsync ? 'async ' : ''}setup(${setupFormalParams}) {\n`,
  )
  ms.appendRight(lastStmt.end!, '\n}')
}
...
export function generatePropsOptions(
  ...
  const segementsFromProps = Object.entries(vineCompFnCtx.props).map(
    ([propName, propMeta]) => {
      const metaFields = []
      ...
      if (propMeta.isMaybeBool) {
        ...
        if (propMeta.default) {
          metaFields.push(
            `default: ${vineFileCtx.getAstNodeContent(propMeta.default)}`,
          )
        }
      }
      if (propMeta.validator) {
        metaFields.push(
          `validator: ${vineFileCtx.originCode.slice(
      ...
      return `${
        propMeta.nameNeedQuoted ? `'${propName}'` : propName
      }: { ${showIf(
  ...
  const segmentsFromVineModel = Object.entries(vineCompFnCtx.vineModels).reduce<
    string[]
  >((segments, [modelName, modelDef]) => {
    const { modelModifiersName, modelOptions } = modelDef
    segments.push(
      `${modelName}: ${
        modelOptions
          ? vineFileCtx.originCode.slice(modelOptions.start!, modelOptions.end!)
          : '{}'
      },`,
    )
  ...
  const propsOptions = [...segementsFromProps, ...segmentsFromVineModel]

  const propsOptionsFields = `${
...
export function generateBasicComponentOptions(
  ...
  const componentName =
    vineCompFnCtx.fnName === 'default' || vineCompFnCtx.fnName === ''
      ? '__VineCompDefault'
...
export function generateDefineComponentWrapper(
  ...
  ms.prependLeft(
    firstStmt.start!,
    `const __vine = _${DEFINE_COMPONENT_HELPER}({\n`,
  )
  ms.appendRight(lastStmt.end!, '\n})')
}
...
export function generateVineFactory(
  ...
  const {
    ...
    vineFileCtx,
    compilerHooks,
    templateComposer: { generatedPreambleStmts, templateCompileResults },
  } = transformCtx
  const { vineCompFnCtx, firstStmt, lastStmt } = fnTransformCtx
  const ms = transformCtx.vineFileCtx.fileMagicCode
  ...
  const explicitExports = fineAllExplicitExports(
    vineFileCtx.exportNamedDeclarations,
  )
  ...
  const isExportDefaultAnonymous =
    vineCompFnCtx.isExportDefault &&
    (vineCompFnCtx.fnName === 'default' || vineCompFnCtx.fnName === '')
  const tempVarName = isExportDefaultAnonymous
    ? '__VineCompDefault'
    : vineCompFnCtx.fnName
  ...
  ms.prependLeft(
    ...
    `${
      ...
      generatedPreambleStmts.get(vineCompFnCtx)?.join('\n') ?? ''
    }\n`,
  )
  ...
  if (vineCompFnCtx.isCustomElement) {
    ...
    ms.appendRight(
      lastStmt.end!,
      `__vine.styles = [${`typeof ${compStyle} === 'undefined' ? null : ${compStyle}`}].filter(Boolean);\n`,
    )
  }
...
export function generateAllImports(transformCtx: TransformContext): void {
  const { mergedImportsMap } = transformCtx
  const ms = transformCtx.vineFileCtx.fileMagicCode
  ...
  for (const [importSource, importMeta] of mergedImportsMap) {
    if (importMeta.type === 'namedSpecifier') {
      ...
      const importStmt = `import { ${specifiers
        .map(
          ([specifier, alias]) =>
            `${specifier}${showIf(Boolean(alias), ` as ${alias}`)}`,
        )
