...
interface DefaultImportSpecifierMeta {
  type: 'defaultSpecifier'
  localName: string
}
interface NamespaceImportSpecifierMeta {
  type: 'namespaceSpecifier'
  localName: string
}
export interface NamedImportSpecifierMeta {
  type: 'namedSpecifier'
...
function getTransformBareAttrAsBoolPlugin(
  ...
  return enableTransformAssetsURL ? [] : []
}
...
export function compileVineTemplate(
...
): (CodegenResult & { templateParsedAst?: ReturnType<typeof parse> }) | null {
  ...
  try {
    ...
    const compileResult = {
      ...
      templateParsedAst: getParsedAst
...
export function createTemplateCompileOptions(
  vineFileCtx: VineFileCtx,
  vineCompFnCtx: VineCompFnCtx,
  compilerHooks: VineCompilerHooks,
  inline: boolean = true,
      ...
      compilerHooks.onError(
        vineErr(
          { vineFileCtx, vineCompFnCtx },
          {
            ...
            location:
              e.loc &&
              computeTemplateErrLocation(vineFileCtx, vineCompFnCtx, e.loc),
  ...
  for (const importStmt of importStmts) {
    ...
    for (const spec of allImportSpecs) {
      if (isImportSpecifier(spec)) {
        const specName = isIdentifier(spec.imported)
          ? spec.imported.name
          : spec.imported.value
        const specAlias = isIdentifier(spec.local) ? spec.local.name : specName
        const specMeta = mergedImportsMap.get(source)
        if (!specMeta) {
          mergedImportsMap.set(source, {
            type: 'namedSpecifier',
...
function getQuasisNode(vineFnCompCtx: VineCompFnCtx) {
...
export function createInlineTemplateComposer(
  ...
  return {
    ...
    compileSetupFnReturns: ({
      ...
      saveImportSpecifier(
        mergedImportsMap,
        preambleAst.program.body.filter((stmt): stmt is ImportDeclaration =>
      ...
      preambleStmts.forEach((stmt) =>
        appendToMapArray(
          generatedPreambleStmts,
          vineCompFnCtx,
          preamble.slice(stmt.start!, stmt.end!),
        ),
      )
...
//           generatedPreambleStmts,
//           vineCompFnCtx,
//           code.slice(codeStmt.start!, codeStmt.end!),
//         )
//       }
...
//           code
//             .slice(exportRenderFnNode.start!, exportRenderFnNode.end!)
//             .replace(
...
//       const allBindings: Record<string, any> = { ...vineCompFnCtx.bindings }
//       for (const key in vineFileCtx.userImports) {
//         const isType = vineFileCtx.userImports[key].isType
//         const isUsedInTemplate =
//           vineFileCtx.userImports[key].isUsedInTemplate?.(vineCompFnCtx)

//         if (isUsedInTemplate) {
//           if (!isType) allBindings[key] = true
//         } else {
...
//         if (
//           allBindings[key] === false ||
//           vineCompFnCtx.bindings[key] === VineBindingTypes.PROPS
//         ) {
...
//         } else if (vineCompFnCtx.bindings[key] === VineBindingTypes.SETUP_LET) {
