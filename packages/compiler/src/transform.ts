...
function transformSimpleArrowFunction(
  ...
  const componentName = isExportDefaultAnonymous
    ? '__VineCompDefault'
    : vineCompFnCtx.fnName
  ...
  const renderFn =
    templateComposer.templateCompileResults.get(vineCompFnCtx) || ''
  ...
  const hmrCreateRecord =
    isDev &&
    compilerHooks.getCompilerCtx().options.runtimeAdapter?.name !== 'rspack'
...
export function transformFile(
  ...
  const templateComposer = createInlineTemplateComposer(compilerHooks, ssr)
  ...
  for (const vineCompFnCtx of vineFileCtx.vineCompFns) {
    ...
    const fnTransformCtx: SingleFnCompTransformCtx = {
      ...
      hasAwait: false,
      vueImportsSpecs: createVueImportsSpecs(transformCtx, vineCompFnCtx),

      isPrependedUseDefaults: false,
      isNeedUseDefaults: Object.values(vineCompFnCtx.props).some((meta) =>
        Boolean(meta.default),
      ),
    }
